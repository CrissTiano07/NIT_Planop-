...
const NIT_PLANOP = (() => {
  ...
  // Letra do turno para o avatar. As iniciais da pessoa eram redundantes —
  // o nome completo aparece sempre ao lado, em todos os contextos.
  const TURNO_LETRA = { manha:'M', tarde:'T', noite:'N' };
  const turnoLetraDe = rId => TURNO_LETRA[S.recursos?.[rId]?.turno_padrao] || '·';

  // Avatar unificado: anel na cor do turno + letra.
  // Codificação redundante (cor + letra) atende quem não distingue matizes,
  // e o anel usa ~85% menos área colorida que o círculo preenchido.
  // Um único helper para os dois contextos (lista de efetivo e chips)
  // — evita a divergência que já causou bug nos chips antes.
  const avatarHTML = (rId, nome, classe = 'staff-avatar', faltou = false) => {
    const letra = turnoLetraDe(rId);
    if (faltou) {
      return `<div class="${classe} av-faltou" title="Faltou">${letra}</div>`;
    }
    const cor = avatarColorFor(rId, nome);
    return `<div class="${classe}" style="border-color:${cor};color:${cor}">${letra}</div>`;
  };
  ...
  const UI = {
    ...
    // Expõe a função livre `toast` (def. no escopo da IIFE) como NIT_PLANOP.UI.toast.
    // Consumido externamente por planop-p0.js (feedback de data inválida no blur).
    toast,
        ...
        chipsDiv.innerHTML = Object.entries(window._pfOriSelecionados)
          .map(([id, nome]) => `
            <div class="pf-ori-chip">
              ${avatarHTML(id.replace(/^[av]:/,''), nome, 'chip-avatar')}
    ...
    _resultadoPessoasHTML(busca, dataVista) {
      ...
      const linhas = achados.map(([rId, r]) => {
        ...
        return `<div class="busca-pessoa-row">
          ${avatarHTML(rId, r.nome, 'staff-avatar')}
          <div class="bp-info">
    ...
    _qruCardHTML(postoId, posto) {
      ...
      const chipsHTML = orientadores.map(([rId,ori]) => {
        ...
        return `<div class="orientador-chip ${ori.faltou?'chip-falta':''}"
          ...
          ${avatarHTML(rId, nome, 'chip-avatar', ori.faltou)}
...
/* ═══════════════════════════════════════════════════════════════════════════
   P0 · DATAS EM FORMATO BRASILEIRO (dd/mm/aaaa)
   ───────────────────────────────────────────────────────────────────────────
   Incorporado ao planop.js (antes era planop-p0.js, arquivo separado).
   Motivo da incorporação: manter a estrutura de três arquivos — sem nome novo,
   sem <script> extra no index.html, sem 404.

   Problema: <input type="date"> renderiza mm/dd/yyyy conforme o idioma do
   navegador, não conforme lang="pt-BR". O operador digitava dia e mês
   invertidos, em silêncio.

   Solução: campo de texto com máscara dd/mm/aaaa, mas .value continua
   devolvendo ISO (YYYY-MM-DD). O resto do planop.js lê/escreve como antes.
   Roda como IIFE própria após a principal — NIT_PLANOP.UI.toast já existe.
   ═══════════════════════════════════════════════════════════════════════════ */
(function () {
  'use strict';
  ...
  function validar(el, aoSair) {
    ...
    if (aoSair && !ok && window.NIT_PLANOP && NIT_PLANOP.UI && NIT_PLANOP.UI.toast) {
      NIT_PLANOP.UI.toast('Data inválida. Use dia/mês/ano — ex.: 05/08/2026', 'danger');
    }
  }
